...
namespace s21
{
template <typename T> class MultiSetIterator {
  public:
    using value_type = T;
  ...
  template <typename T> class MultietConstIterator {
  public:
    using value_type = T;
...
public:
  ...
  iterator find(const key_type &key) { return MultiSetIterator(root_->find(key)); };
