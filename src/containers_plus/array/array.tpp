...
template <typename T, std::size_t N> constexpr array<T, N>::array(array &&a) {
  if (a.max_size() != capacity_)
    throw std::invalid_argument("#");
  for (size_t i = 0; i != a.size(); ++i)
    storage_[i] = std::move(a[i]);
  size_ = a.size();
}
...
template <typename T, std::size_t N>
constexpr array<T, N> &
array<T, N>::operator=(const std::initializer_list<value_type> &values) {
...
array<T, N>::operator[](const size_type &pos) {
...
array<T, N>::operator[](const size_type &pos) const {
  // if (pos > capacity_) throw std::out_of_range("#");
  return /*pos > size_ ? 0 :*/ storage_[pos];
}
...
template <typename T, std::size_t N>
constexpr typename array<T, N>::reference
array<T, N>::at(const size_type &pos) {
  if (pos > capacity_)
...
template <typename T, std::size_t N>
constexpr typename array<T, N>::const_reference
array<T, N>::at(const size_type &pos) const {
  if (pos > capacity_)
    throw std::out_of_range("#");
  return /*pos > size_ ? 0 :*/ storage_[pos];
}
...
template <typename T, std::size_t N>
constexpr typename array<T, N>::const_iterator
...
template <typename T, std::size_t N>
constexpr typename array<T, N>::const_iterator
array<T, N>::cbegin() const noexcept { // returns an iterator to the beginning
  return storage_;
}
...
template <typename T, std::size_t N>
constexpr typename array<T, N>::const_iterator
array<T, N>::cend() const noexcept { // returns an iterator to the end
  return storage_ + capacity_;
}
